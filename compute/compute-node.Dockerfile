...
# pg_config. For some reason the cmake scripts work with just that. By also
# adding /usr/local/pgsql, it works, which is weird because there are no executables
# in that directory.
